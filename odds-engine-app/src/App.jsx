...
const LEVELS = [
  ...
  { key: "associate", label: "Associate", adj: -5 },
  { key: "experienced", label: "Experienced / Mid-Career", adj: -8 },
];
const OFFICES = [
...
function computeScore(answers, weights) {
  ...
  const factors = [
    { label: "Firm / Fund Tier", value: tierAdj, note: sector.tiers[tierIndex] },
    { label: "Role Level Fit", value: level.adj, note: level.label },
    { label: "Office Market", value: office.adj, note: office.label },
    { label: "Resume Keyword Alignment", value: resumeBoost, note: `${Math.round(resumeMatch*100)}% match` },
    { label: "Application Timing", value: -timingPenalty, note: cycle.label },
    { label: "Visa / Work Authorization", value: -visaPenalty, note: visa.label },
  ];
  ...
  return { total, band, base, factors, sector };
}
...
const MOCK_EXPERIENCE = {
  ib: "Summer Analyst — Bulge Bracket Bank, M&A Coverage Group",
  consulting: "Summer Associate — Tier-2 Strategy Consultancy",
  pe: "Investment Analyst Intern — Growth Equity Fund",
  re: "Acquisitions Analyst Intern — Regional Development Firm",
};
function buildMockExtraction(sector) {
  const found = sector.keywords.slice(0, 5);
  return {
    name: "Jordan A. Whitfield",
    contact: "jordan.whitfield@email.com  ·  (555) 013-2847",
    education: "B.S. Finance, University of Michigan — GPA 3.7 / 4.0",
    experience: MOCK_EXPERIENCE[sector.key] || "Relevant internship experience detected",
    targetFirmsFound: sector.tiers,
    skills: found.map(prettyKeyword),
    fraction: Math.min(1, found.length / sector.keywords.length),
  };
}
...
/* ============================== ADMIN VIEW ============================== */
function AdminView({ config, setConfig, tokens, onLock, embedded }) {
...
function AssessmentView({ config, tokens, answers, setAnswers, setView }) {
  ...
  const [extractStatus, setExtractStatus] = useState("idle"); // idle | extracting | done
  const [mockExtraction, setMockExtraction] = useState(null);
  ...
  const handleFile = (f) => {
    ...
    setFileName(f.name);
    setMockExtraction(null);
    setExtractStatus("extracting");
    setTimeout(() => {
      setMockExtraction(buildMockExtraction(sector));
  ...
  const finish = () => {
    let resumeMatch = 0.4;
    if (mockExtraction) resumeMatch = mockExtraction.fraction;
    else {
      const m = scanResume(resumeText, sector);
      if (m) resumeMatch = m.fraction;
    }
    setAnswers(a => ({ ...a, resumeText, resumeMatch, submitted: true }));
    setView("results");
  };
      ...
      <div style={card} className="p-8">
        ...
        {step === 3 && (
          <div>
            ...
            {extractStatus === "done" && mockExtraction && (
              <div className="mb-5" style={{ border: `1px solid ${tokens.border}`, borderRadius: tokens.radius, padding: 20 }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <div style={{ fontSize: 13.5, color: tokens.text }}>{mockExtraction.name}</div>
                    <div style={{ fontSize: 12, color: tokens.textMuted }}>{mockExtraction.contact}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: tokens.textMuted, marginBottom: 3 }}>Education &amp; GPA</div>
                    <div style={{ fontSize: 13.5, color: tokens.text }}>{mockExtraction.education}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: tokens.textMuted, marginBottom: 3 }}>Work Experience</div>
                    <div style={{ fontSize: 13.5, color: tokens.text }}>{mockExtraction.experience}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: tokens.textMuted, marginBottom: 3 }}>Target Firm Tiers Found</div>
                    <div style={{ fontSize: 13.5, color: tokens.text }}>{mockExtraction.targetFirmsFound.join(" · ")}</div>
                  </div>
                </div>
                <div style={{ fontSize: 11, color: tokens.textMuted, marginBottom: 6 }}>Extracted Skill Keywords</div>
                <div className="flex flex-wrap gap-2">
                  {mockExtraction.skills.map(k => (
                ...
                <div style={{ fontSize: 10.5, color: tokens.textMuted, marginTop: 12 }}>Simulated extraction for prototype purposes — not a real parse of the uploaded file.</div>
              </div>
            )}
...
function AdminDrawer({ isOpen, onClose, config, setConfig, tokens, isAdminAuthenticated, setIsAdminAuthenticated }) {
  return (
