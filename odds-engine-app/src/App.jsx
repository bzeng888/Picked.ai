...
const CYCLES = [
  { key: "peak", label: "Early / On-Cycle (Peak)", factor: 0 },
  { key: "standard", label: "Standard Window", factor: 0.35 },
  { key: "late", label: "Late Submission", factor: 0.7 },
  { key: "off", label: "Off-Cycle", factor: 1 },
];
const VISAS = [
...
function computeScore(answers, weights) {
  ...
  // not a level-mismatch problem, so a detected bridge is scored as its own explicit +10% to +20% positive lever.
  const hasConsultingBridge = sector.key !== "consulting" && !!answers.hasConsultingBridge;
  const bridgeMatches = hasConsultingBridge ? (answers.bridgeMatches || []) : [];
  const bridgeFraction = Math.min(1, bridgeMatches.length / 6);
  const bridgeBoost = hasConsultingBridge ? Math.max(10, Math.min(20, Math.round(10 + bridgeFraction * 10))) : 0;
  ...
  const factors = [
    ...
    { label: "Consulting Background Bridge", value: bridgeBoost,
      note: hasConsultingBridge ? "Detected — transferable strategy/advisory experience" : "No consulting background detected",
  ...
  return { total, band, base, factors, sector, bridgeMatches, hasConsultingBridge, gpaValue };
}
/**
 * Scenario calculator: reuses the full scoring engine with overridden tier/timing/office/resume-match
 * values, so results always stay consistent with the Results Dashboard for identical inputs, then
 * layers a flat networking/referral bonus on top (a lever the formal intake flow doesn't collect).
 */
function computeScenarioTotal(baselineAnswers, overrides, weights) {
  const merged = { ...baselineAnswers, ...overrides };
  const base = computeScore(merged, weights);
  const referralBoost = overrides.hasReferral ? 8 : 0;
  const total = Math.max(1, Math.min(97, base.total + referralBoost));
  return { ...base, total, referralBoost };
}
...
function buildStrengthHighlight(result) {
  if (!result.hasConsultingBridge) return null;
...
/** Word-boundary substring match — avoids false positives on short tokens (e.g. "ey" inside "they"). */
function containsKeyword(lowerText, keyword) {
  return new RegExp(`\\b${escapeRegExp(keyword.toLowerCase())}\\b`, "i").test(lowerText);
}
function scanResume(text, sector) {
  if (!text || text.trim().length < 20) return null;
  const t = text.toLowerCase();
  const found = sector.keywords.filter(k => containsKeyword(t, k));
...
const KEYWORD_LABELS = {
  ...
  // Consulting firm names & tiers
  mckinsey: "McKinsey & Company", bcg: "BCG", bain: "Bain & Company", deloitte: "Deloitte",
  pwc: "PwC", ey: "EY", kpmg: "KPMG", "oliver wyman": "Oliver Wyman", lek: "L.E.K. Consulting",
  "roland berger": "Roland Berger", "strategy&": "Strategy&", "accenture strategy": "Accenture Strategy",
  // Consulting skill keywords
  "management consulting": "Management Consulting", "client advisory": "Client Advisory",
  "cross-functional leadership": "Cross-Functional Leadership", "project management": "Project Management",
  "market entry": "Market Entry", "operations": "Operations", "business transformation": "Business Transformation",
};
...
const CONSULTING_FIRM_KEYWORDS = [
  "mckinsey", "bcg", "bain", "deloitte", "pwc", "ey", "kpmg",
  "oliver wyman", "lek", "roland berger", "strategy&", "accenture strategy",
];
const CONSULTING_SKILL_KEYWORDS = [
  "management consulting", "strategy", "client advisory", "cross-functional leadership",
  "project management", "financial modeling", "due diligence", "market entry", "valuation",
  "operations", "business transformation", "case study", "hypothesis-driven", "frameworks",
  "stakeholder", "client engagement", "cross-functional", "client delivery", "change management",
];
const CONSULTING_BRIDGE_KEYWORDS = Array.from(new Set([...CONSULTING_FIRM_KEYWORDS, ...CONSULTING_SKILL_KEYWORDS]));

/** Dedicated classifier: does this candidate show a consulting background bridge for a non-consulting target sector? */
function classifyConsultingBridge(lowerText, sectorKey) {
  if (sectorKey === "consulting") return { detected: false, matches: [], firmsFound: [] };
  const matches = CONSULTING_BRIDGE_KEYWORDS.filter(k => containsKeyword(lowerText, k));
  const firmsFound = CONSULTING_FIRM_KEYWORDS.filter(k => containsKeyword(lowerText, k));
  return { detected: matches.length > 0, matches, firmsFound };
}
...
function analyzeResumeText(rawText, sector, fileName) {
  ...
  const matched = allKeywords.filter(k => containsKeyword(lower, k));
  const tiersFound = sector.tiers.filter(t => containsKeyword(lower, t));
  const bridge = classifyConsultingBridge(lower, sector.key);
  ...
  return {
    ...
    hasConsultingBridge: bridge.detected,
    firmsFound: bridge.firmsFound,
    gpaValue,
...
function NavBar({ view, setView, tokens, copy }) {
  const tabs = [
    ...
    { key: "scenario", label: "Scenario Calculator", icon: "sliders" },
  ];
  return (
...
function AssessmentView({ config, tokens, answers, setAnswers, setView }) {
  ...
  const finish = () => {
    let resumeMatch = 0.4;
    let bridgeMatches = [];
    let hasConsultingBridge = false;
    let gpaValue = null;
    if (extraction) {
      resumeMatch = extraction.fraction;
      bridgeMatches = extraction.bridgeMatches;
      hasConsultingBridge = extraction.hasConsultingBridge;
      gpaValue = extraction.gpaValue;
    } else {
      const m = scanResume(resumeText, sector);
      if (m) resumeMatch = m.fraction;
      if (resumeText.trim().length > 20) {
        const bridge = classifyConsultingBridge(resumeText.toLowerCase(), sector.key);
        bridgeMatches = bridge.matches;
        hasConsultingBridge = bridge.detected;
        gpaValue = extractGpaValue(resumeText);
      }
    }
    setAnswers(a => ({ ...a, resumeText, resumeMatch, bridgeMatches, hasConsultingBridge, gpaValue, submitted: true }));
    setView("results");
  };
          ...
          <div>
            ...
            {extractStatus === "done" && extraction && (
              <div className="mb-5" style={{ border: `1px solid ${tokens.border}`, borderRadius: tokens.radius, padding: 20 }}>
                ...
                {extraction.hasConsultingBridge && (
                  <div className="mb-4 flex items-start gap-2 px-3 py-2.5" style={{ borderRadius: 8,
                    background: tokens.mode === "dark" ? "rgba(127,203,158,0.1)" : "rgba(47,107,72,0.07)" }}>
                    <Icon name="check" size={14} style={{ color: tokens.positive, marginTop: 1, flexShrink: 0 }} />
                    <div>
                      <div style={{ fontSize: 13, color: tokens.text, fontWeight: 600 }}>Consulting Background Bridge: Detected</div>
                      <div style={{ fontSize: 11.5, color: tokens.textMuted }}>Recognized strategy/advisory experience with transferable skill alignment.</div>
                    </div>
                  </div>
                )}
      ...
      {strengthHighlight && (
        <div className="mb-10" style={{ ...card, padding: 22, borderLeft: `3px solid ${tokens.accent}` }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span style={{ fontSize: 14, fontFamily: tokens.fonts.display, color: tokens.text }}>Consulting Background Bridge: Detected</span>
          </div>
          <div style={{ fontSize: 12, color: tokens.textMuted, marginBottom: 14, marginLeft: 22 }}>
            Recognized strategy/advisory experience with transferable skill alignment.
          </div>
          <div className="flex items-center gap-2 mb-3">
            <span style={{ fontSize: 11, fontFamily: FONT_MONO, color: tokens.textMuted, letterSpacing: "0.04em" }}>STRENGTH HIGHLIGHTS</span>
          </div>
...
/* ============================== SCENARIO CALCULATOR (WHAT-IF) ============================== */
function SegmentedControl({ tokens, options, value, onChange }) {
  return (
    <div className="flex flex-wrap gap-2">
      {options.map(opt => (
        <button key={opt.key} onClick={() => onChange(opt.key)}
          ...
          style={{
            border: `1px solid ${value === opt.key ? tokens.accent : tokens.border}`,
            background: value === opt.key ? (tokens.mode === "dark" ? "rgba(176,141,87,0.16)" : "rgba(176,141,87,0.08)") : "transparent",
            borderRadius: tokens.radius, color: tokens.text, transition: "all .2s ease",
          }}>
          {opt.label}
        </button>
      ))}
...
function RationaleBlurb({ tokens, s1, s2 }) {
  return <div style={{ fontSize: 12, color: tokens.textMuted, lineHeight: 1.55, marginTop: 10 }}>{s1} {s2}</div>;
}
const TIER_RATIONALE = [
  { s1: "The top bracket in this sector typically converts under 3% of applicants, so tier alone is the single heaviest lever on your score.",
    s2: "Keep a parallel pipeline of tier-two firms warm in case this bracket's process stalls." },
  { s1: "Upper-tier and elite-boutique processes are still selective, but they issue meaningfully more offers per applicant than the very top bracket.",
    s2: "Naming a specific deal, case win, or sector focus in your application narrows the gap fastest at this tier." },
  { s1: "Mid-tier and regional firms typically run smaller, faster processes with materially higher offer rates.",
    s2: "These firms tend to weight direct outreach and referrals more heavily than a polished resume alone." },
];
const TIMING_RATIONALE = {
  peak: { s1: "Applying on-cycle aligns with peak hiring quotas, when firms issue the large majority of their offer slots for the cycle.",
    s2: "Submitting as early as the window opens maximizes recruiter attention before fatigue sets in later in the cycle." },
  standard: { s1: "A standard-window application still lands inside the main hiring push, just past the earliest wave of submissions.",
    s2: "Follow up with a referral or coffee chat soon after submitting — it helps offset the slight review-order disadvantage." },
  late: { s1: "Late submissions compete for a shrinking pool of remaining seats, since many roles are informally filled by this point.",
    s2: "Target firms explicitly still marked 'actively hiring' rather than applying broadly at this stage." },
  off: { s1: "Off-cycle applications face the steepest odds — most planned headcount for the cycle has already been allocated.",
    s2: "Lean on a direct referral or a need-driven opening rather than the standard pipeline, which is largely closed." },
};
const OFFICE_RATIONALE = {
  tier1: { s1: "Tier-1 hubs draw the deepest applicant pools relative to open seats, which is where the prestige premium is paid for.",
    s2: "A hub-specific reason to be there (school ties, existing network, past internship) helps offset the added competition." },
  tier2: { s1: "Secondary and regional offices run smaller, less crowded processes with meaningfully higher seats-per-applicant ratios.",
    s2: "Say explicitly why you want that specific market — it reads as commitment rather than a fallback choice." },
};
const REFERRAL_RATIONALE = {
  yes: { s1: "A warm introduction changes how an application is actually read — it typically moves a resume from the general queue to a named reviewer.",
    s2: "Ask your contact to flag the application directly rather than just forwarding your resume, which is a materially weaker signal." },
  no: { s1: "Cold applications get read fastest and least generously, competing purely on resume and keyword match.",
    s2: "Even one alumni or second-degree connection at the firm meaningfully changes this lever — it's worth a week of outreach before applying." },
};
function resumeRationale(basePct, adjustedPct) {
  return {
    s1: `Moving your keyword match from ${basePct}% to ${adjustedPct}% closes real ground with resume screens, which filter heavily on exact sector terminology.`,
    s2: "Rewrite bullet points to mirror the target sector's own vocabulary — that moves this number more than adding generic action verbs.",
  };
}
function ScenarioCalculatorView({ config, tokens, answers, setView }) {
  const card = cardSx(tokens, config.theme.cardStyle);
  const sector = SECTORS.find(s => s.key === answers.sector) || SECTORS[0];

  const [tierIndex, setTierIndex] = useState(answers.tierIndex ?? 0);
  const [cycleKey, setCycleKey] = useState(answers.cycle || CYCLES[0].key);
  const [officeKey, setOfficeKey] = useState(answers.office || OFFICES[0].key);
  const [hasReferral, setHasReferral] = useState(false);
  const [resumeBoostPct, setResumeBoostPct] = useState(0);

  if (!answers.submitted) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-24 text-center">
        <Icon name="sliders" size={26} style={{ color: tokens.textMuted }} className="mx-auto mb-4" />
        <h2 style={{ fontFamily: tokens.fonts.display, color: tokens.text, fontSize: 22, marginBottom: 10 }}>No baseline yet</h2>
        <p style={{ color: tokens.textMuted, fontSize: 14, marginBottom: 24 }}>Complete the intake flow first — the calculator starts from your actual results.</p>
        <Btn tokens={tokens} onClick={() => setView("assessment")}>Start assessment</Btn>
      </div>
    );
  }

  const baselineResumeMatch = answers.resumeMatch ?? 0.4;
  const adjustedResumeMatch = Math.min(1, baselineResumeMatch + resumeBoostPct / 100);
  const overrides = { tierIndex, cycle: cycleKey, office: officeKey, resumeMatch: adjustedResumeMatch, hasReferral };

  const baseline = computeScore(answers, config.weights);
  const scenario = computeScenarioTotal(answers, overrides, config.weights);
  const delta = scenario.total - baseline.total;
  const cycle = CYCLES.find(c => c.key === cycleKey) || CYCLES[0];

  const rowLabel = { fontSize: 12.5, color: tokens.textMuted, marginBottom: 8, fontFamily: tokens.fonts.body };

  return (
    <div className="max-w-6xl mx-auto px-6 py-14">
      <SectionLabel tokens={tokens}>SCENARIO CALCULATOR — WHAT-IF SIMULATOR</SectionLabel>
      <p style={{ color: tokens.textMuted, fontSize: 13.5, marginBottom: 28, maxWidth: 640 }}>
        Starts from your actual intake and resume results. Adjust any lever below to see how it moves your probability, live.
      </p>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-3">
          <div style={{ ...card, padding: 24 }} className="mb-6">
            <div style={rowLabel}>1 · TARGET FIRM TIER</div>
            <SegmentedControl tokens={tokens} value={tierIndex}
              options={sector.tiers.map((t, i) => ({ key: i, label: t }))}
              onChange={(v) => setTierIndex(v)} />
            <RationaleBlurb tokens={tokens} {...TIER_RATIONALE[tierIndex]} />
          </div>

          <div style={{ ...card, padding: 24 }} className="mb-6">
            <div style={rowLabel}>2 · APPLICATION TIMING</div>
            <SegmentedControl tokens={tokens} value={cycleKey}
              options={CYCLES.map(c => ({ key: c.key, label: c.label }))}
              onChange={setCycleKey} />
            <RationaleBlurb tokens={tokens} {...TIMING_RATIONALE[cycleKey]} />
          </div>

          <div style={{ ...card, padding: 24 }} className="mb-6">
            <div style={rowLabel}>3 · OFFICE LOCATION TIER</div>
            <SegmentedControl tokens={tokens} value={officeKey}
              options={OFFICES.map(o => ({ key: o.key, label: o.label }))}
              onChange={setOfficeKey} />
            <RationaleBlurb tokens={tokens} {...OFFICE_RATIONALE[officeKey]} />
          </div>

          <div style={{ ...card, padding: 24 }} className="mb-6">
            <div style={rowLabel}>4 · NETWORKING / REFERRAL STATUS</div>
            <SegmentedControl tokens={tokens} value={hasReferral ? "yes" : "no"}
              options={[{ key: "no", label: "Cold Apply (No Referral)" }, { key: "yes", label: "Warm Contact / Referral Secured" }]}
              onChange={(v) => setHasReferral(v === "yes")} />
            <RationaleBlurb tokens={tokens} {...REFERRAL_RATIONALE[hasReferral ? "yes" : "no"]} />
          </div>

          <div style={{ ...card, padding: 24 }}>
            <div className="flex justify-between items-baseline mb-2">
              <div style={rowLabel}>5 · RESUME / SKILL ALIGNMENT MULTIPLIER</div>
              <span style={{ fontFamily: FONT_MONO, fontSize: 12.5, color: tokens.accent }}>+{resumeBoostPct}%</span>
            </div>
            <input type="range" min={0} max={25} value={resumeBoostPct} onChange={e => setResumeBoostPct(Number(e.target.value))}
              style={{ "--thumb": tokens.accent, width: "100%" }} />
            <div className="flex justify-between mt-1" style={{ fontSize: 10.5, color: tokens.textMuted }}>
              <span>Baseline Resume</span><span>Optimized / Tailored Keywords</span>
            </div>
            <RationaleBlurb tokens={tokens} {...resumeRationale(Math.round(baselineResumeMatch * 100), Math.round(adjustedResumeMatch * 100))} />
          </div>
        </div>

        <div className="lg:col-span-2">
          <div style={{ ...card, padding: 28, position: "sticky", top: 84 }} className="flex flex-col items-center text-center">
            <div style={{ fontSize: 11, fontFamily: FONT_MONO, color: tokens.textMuted, letterSpacing: "0.05em", marginBottom: 16 }}>LIVE ADJUSTED SCORE</div>
            <Gauge percent={scenario.total} tokens={tokens} />
            <div style={{ marginTop: 10, padding: "5px 14px", borderRadius: 20, fontSize: 12.5, fontFamily: FONT_MONO,
              background: tokens.mode === "dark" ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.05)", color: tokens.text }}>
              {scenario.band}
            </div>

            <div className="w-full mt-6 pt-5" style={{ borderTop: `1px solid ${tokens.border}` }}>
              <div style={{ fontSize: 11, color: tokens.textMuted, marginBottom: 6 }}>VS. YOUR BASELINE ({baseline.total}%)</div>
              <div style={{ fontFamily: FONT_MONO, fontSize: 26, fontWeight: 600, transition: "color .3s ease",
                color: delta > 0 ? tokens.positive : delta < 0 ? tokens.negative : tokens.textMuted }}>
                {delta > 0 ? "+" : ""}{delta}% {delta > 0 ? "Net Increase" : delta < 0 ? "Net Decrease" : "No Change"}
              </div>
            </div>

            {hasReferral && (
              <div className="w-full mt-4 flex justify-between" style={{ fontSize: 12, color: tokens.textMuted }}>
                <span>Referral bonus</span>
                <span style={{ fontFamily: FONT_MONO, color: tokens.positive }}>+{scenario.referralBoost}%</span>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <AdminContext.Provider value={{ config, setConfig }}>
      <div style={{ minHeight: "100vh", background: tokens.bg, fontFamily: tokens.fonts.body }}>
        ...
        {view === "scenario" && <ScenarioCalculatorView config={config} tokens={tokens} answers={answers} setView={setView} />}
