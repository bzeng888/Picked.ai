...
function guessCandidateName(text) {
  ...
  for (const line of lines) {
    if (line.length < 40 && /^[A-Z][a-zA-Z.'-]+(\s+[A-Z][a-zA-Z.'-]+){1,3}$/.test(line)) return line;
  }
  return null;
}

/** Scans real extracted text for sector + general keywords and pulls light contact/education signals. */
function analyzeResumeText(rawText, sector) {
  ...
  return {
    ...
    name: name || "Not detected — add a clear name line at the top of the document",
    ...
    education: gpaMatch
      ? `${eduMatch ? eduMatch[0].trim() : "Degree detected"} — GPA ${gpaMatch[1]}`
...
function AssessmentView({ config, tokens, answers, setAnswers, setView }) {
  ...
  const handleFile = async (f) => {
    ...
    try {
      ...
      setExtraction(analyzeResumeText(text, sector));
  ...
  const finish = () => {
    let resumeMatch = 0.4;
    if (extraction) resumeMatch = extraction.fraction;
    else {
